...
```{r}
gvc.genes.minus_apoe_hla %>% distinct(gene_symbol) %>% nrow()
```

Remove genes farther than 200Kb from GVC loci:
...
```{r}
gvc.genes.minus_apoe_hla.200kb %>% distinct(gene_symbol) %>% nrow()
```
...
### GVC genes unsorted/unfiltered minus genes in APOE and HLA loci
