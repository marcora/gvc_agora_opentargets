...
- [ ] Redo ORA of GVC genes excluding MHC and APOE locus genes
- [ ] Summarize ORA results using `enrichplot`, `clusterProfiler`, or similar tools
...
```{r}
gvc.minus_apoe_hla <- gvc %>%
  filter(
    grouped_loci_gvc != "APOE / TOMM40" & grouped_loci_gvc != "HLA") %>%
  bind_rows(tibble(
    gene_id = c("ENSG00000130203", "ENSG00000196735", "ENSG00000179344", "ENSG00000196126"),
    gene_symbol = c("APOE", "HLA-DQA1", "HLA-DQB1", "HLA-DRB1")))
```

```{r}
gvc.genes.minus_apoe_hla <- gvc.minus_apoe_hla %>% distinct(gene_id, .keep_all = TRUE) %>% select(gene_id, gene_symbol) %>% arrange(gene_symbol)
...
```{r}
gvc.genes.minus_apoe_hla.200kb <- gvc.minus_apoe_hla.200kb %>% distinct(gene_id, .keep_all = TRUE) %>% select(gene_id, gene_symbol) %>% arrange(gene_symbol)

gvc.genes.minus_apoe_hla.200kb
```
...
```{r}
gvc.minus_apoe_hla.20kb <- gvc %>%
  filter(
    absolute_distance <= 20000,
    grouped_loci_gvc != "APOE / TOMM40" & grouped_loci_gvc != "HLA") %>%
  bind_rows(tibble(
    gene_id = c("ENSG00000130203", "ENSG00000196735", "ENSG00000179344", "ENSG00000196126"),
    gene_symbol = c("APOE", "HLA-DQA1", "HLA-DQB1", "HLA-DRB1")))
...
```{r}
gvc.genes.minus_apoe_hla.20kb <- gvc.minus_apoe_hla.20kb %>% distinct(gene_id, .keep_all = TRUE) %>% select(gene_id, gene_symbol) %>% arrange(gene_symbol)

gvc.genes.minus_apoe_hla.20kb
```
...
## Correlation between Agora and OpenTargets scores
...
```{r}
d0.minus_apoe_hla.200kb <- gvc.genes.minus_apoe_hla.200kb %>% select(gene_id, gene_symbol)
...
```{r}
d0.minus_apoe_hla.20kb <- gvc.genes.minus_apoe_hla.20kb %>% select(gene_id, gene_symbol)
...
### GVC genes (within 1Mb flanking regions of GVC loci) minus APOE and HLA loci genes, filtered and sorted by Agora's `genetics_score`
...
### GVC genes (within 1Mb flanking regions of GVC loci) minus APOE and HLA loci genes, filtered and sorted by OpenTargets' `otGeneticsPortal`
...
### GVC genes (within 1Mb flanking regions of GVC loci) minus APOE and HLA loci genes, filtered and sorted by Agora's `target_risk_score`
...
### GVC genes (within 1Mb flanking regions of GVC loci) minus APOE and HLA loci genes, filtered and sorted by OpenTargets' `globalScore`
