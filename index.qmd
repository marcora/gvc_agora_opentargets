...
-   [x] Redo ORA of GVC genes excluding MHC and APOE locus genes
...
Perform alternative ORA of GO:CP using [GOrilla](https://cbl-gorilla.cs.technion.ac.il/) \<2024-10-24\>:

```{r}
d0.minus_apoe_hla %>% distinct(gene_id) %>% pull(gene_id) %>% write("query.txt")
```

![](gvc.genes.minus_apoe_hla.gorilla.png)
...
```{r}
weights <- c(0.5, 0.25, 0.25)  # weights for gvc, agora, open_targets ranks
...
d <- gvc.genes.minus_apoe_hla %>%
  ...
  select(gene_id, gene_symbol, combined_rank, gvc_rank, agora_rank, opentargets_rank, absolute_distance, target_risk_score, globalScore, everything())
