...
x = list(
  ...
  "Open Targets" = otensg$ensembl_gene_id
)
```
...
overlap.genes <- p |>
  unnest(..values..) |>
  filter(..set.. == "GVC∩Agora∩Open Targets") |> 
  select(gene_id = ..values..) |> 
  left_join(gvc.genes, by = "gene_id")
...
```{r}
query <- overlap.genes |> distinct(gene_symbol) |> pull(gene_symbol)
